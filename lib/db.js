...
export async function getMembers() {
  ...
  for (const m of members) {
    ...
    let statusFisik = "✅ SEIMBANG";
    let statusCode = "seimbang";
    let tindakLanjut = saldoBuku > 0 ? `Setor sisa Rp ${formatNumber(saldoBuku)} ke Kasir` : "Sudah Tertentuk";

    if (selisih < 0) {
      statusFisik = "❌ KEKURANGAN";
      statusCode = "kekurangan";
      tindakLanjut = `Wajib mengganti kekurangan Rp ${formatNumber(Math.abs(selisih))}`;
    } else if (selisih > 0) {
      statusFisik = "⚠️ BERLEBIHAN";
      statusCode = "berlebihan";
...
export async function getSummary() {
  ...
  let statusSelisihTim = "✅ SELURUH ANGGOTA SEIMBANG";
  let tindakLanjutTim = "Semua fisik sesuai catatan";
  if (totalSelisihFisik < 0) {
    statusSelisihTim = `❌ TOTAL KEKURANGAN Rp ${formatNumber(Math.abs(totalSelisihFisik))}`;
    tindakLanjutTim = "Investigasi kekurangan fisik!";
  } else if (totalSelisihFisik > 0) {
    statusSelisihTim = `⚠️ TOTAL BERLEBIHAN Rp ${formatNumber(totalSelisihFisik)}`;
  ...
  if (auditGap < 0) {
    statusAudit = `❌ KEKURANGAN FISIK Rp ${formatNumber(Math.abs(auditGap))}`;
    auditCode = "kekurangan";
  } else if (auditGap > 0) {
    statusAudit = `⚠️ BERLEBIHAN FISIK Rp ${formatNumber(auditGap)}`;
    auditCode = "berlebihan";
  }
  return {
